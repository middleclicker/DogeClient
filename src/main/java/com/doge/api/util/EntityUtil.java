...
public class EntityUtil implements Global {
    ...
    public static List<Vec3d> getBlockBlocks(Entity entity) {
        ...
        double minX = round((float) bb.minX, 0);
        double minZ = round((float) bb.minZ, 0);
        double maxX = round((float) bb.maxX, 0);
        double maxZ = round((float) bb.maxZ, 0);
        if (minX != maxX) {
    ...
    public static Vec3d roundVec(Vec3d vec3d, int places) {
        return new Vec3d(round((float) vec3d.x, places), round((float) vec3d.y, places), round((float) vec3d.z, places));
    }
    ...
    public static boolean isntValid(Entity entity, double range) {
        return entity == null || EntityUtil.isDead(entity) || entity.equals(EntityUtil.mc.player) || entity instanceof EntityPlayer && FriendManager.isFriend(entity.getName()) || EntityUtil.mc.player.getDistanceSq(entity) > range*range;
    }
    ...
    public static Map<String, Integer> getTextRadarPlayers() {
        ...
        for (EntityPlayer player : EntityUtil.mc.world.playerEntities) {
            ...
            output.put(healthSB.toString() + " " + (FriendManager.isFriend(player.getName()) ? ChatFormatting.AQUA : ChatFormatting.RED) + player.getName() + " " + distanceSB.toString() + " \u00c2\u00a7f0", (int) EntityUtil.mc.player.getDistance(player));
