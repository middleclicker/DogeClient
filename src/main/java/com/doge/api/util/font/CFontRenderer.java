...
public class CFontRenderer extends CFont {
    ...
    public float drawStringWithShadow(String text, double x, double y, DColor color) {
        float shadowWidth = drawString(text, x + 1D, y + 1D, color, true);
        return Math.max(shadowWidth, drawString(text, x, y, color, false));
    }

    public float drawString(String text, float x, float y, DColor color) {
        return drawString(text, x, y, color, false);
    }

    public float drawCenteredStringWithShadow(String text, float x, float y, DColor color) {
        return drawStringWithShadow(text, x - getStringWidth(text) / 2, y, color);
    }

    public float drawCenteredString(String text, float x, float y, DColor color) {
        return drawString(text, x - getStringWidth(text) / 2, y, color);
    }

    public float drawString(String text, double x, double y, DColor dColor, boolean shadow) {
        x -= 1;
        y -= 2;
        DColor color = new DColor(dColor);
        if (text == null) return 0.0F;
        if (color.getRed() == 255 && color.getGreen() == 255 && color.getBlue() == 255 && color.getAlpha() == 32)
            color = new DColor(255, 255, 255);
        if (color.getAlpha() < 4) color = new DColor(color, 255);
        if (shadow)
            color = new DColor(color.getRed() / 4, color.getGreen() / 4, color.getBlue() / 4, color.getAlpha());
